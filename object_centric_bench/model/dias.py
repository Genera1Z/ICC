...
class ARRandTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        """ ### <<< detach slots until the last layer: always enabled vs scheduled from yes to no -- bad
        def detach_slots_hook(module, inputs):
            inputs = list(inputs)
            if len(inputs) > 1 and inputs[1] is not None:
                inputs[1] = inputs[1].detach()
            return tuple(inputs)
        for layer in self.backbone.layers[:-1]:
            layer.register_forward_pre_hook(detach_slots_hook)
        ### >>> """
